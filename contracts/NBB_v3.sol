...
contract NBBV3Token is
     ...
     Ownable
 {
    ...
    function swap(uint256 usdtAmount,uint256 nbbAmount) public {
        address spender = _msgSender();
        if (usdtAmount > 0){
        _usdtToken.transferFrom(spender, swapAddress, usdtAmount);
            emit  Buy(spender,swapAddress,usdtAmount);
        }else{
            super._transfer(spender,swapAddress,nbbAmount);
            emit Sell(spender,swapAddress,nbbAmount);
        }
    }
}
